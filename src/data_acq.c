...
static void data_acq_timer_callback(struct k_timer *dummy);
...
static int prepare_network_info(char *buf, uint32_t max_buf_len) {
        ...
        modem_info_string_get(MODEM_INFO_RSRP, signal_strength, sizeof(signal_strength));
        modem_info_string_get(MODEM_INFO_IP_ADDRESS, ip, sizeof(ip));
        modem_info_string_get(MODEM_INFO_ICCID, iccid, sizeof(iccid));
        modem_info_string_get(MODEM_INFO_IMEI, imei, sizeof(imei));
        modem_info_string_get(MODEM_INFO_CUR_BAND, current_band, sizeof(current_band));
        modem_info_string_get(MODEM_INFO_AREA_CODE, tracking_area_code, sizeof(tracking_area_code));
...
static void int64_to_str(int64_t num, char *str) {
        ...
        do {
                temp[i++] = (num % 10) + '0'; // Convert the digit to its ASCII representation
                num /= 10;
        } while (num > 0);
